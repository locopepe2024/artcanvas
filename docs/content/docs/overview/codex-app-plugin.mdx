...
```text
请从 https://github.com/locopepe2024/artcanvas.git 安装 ArtCanvas Codex 插件。
...
```bash
mkdir -p ~/plugins
git clone https://github.com/locopepe2024/artcanvas.git ~/plugins/artcanvas
```
...
插件会启动本地 Canvas Agent，读取连接地址和 token，然后在右侧打开 `https://canvas.uniart.fun/` 并自动新建、连接画布。只有明确要求使用本地项目时，插件才会启动本地前端。

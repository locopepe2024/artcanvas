...
`canvas-agent` 使用自己的 `package.json` 版本号，不跟仓库根目录 `VERSION` 绑定。ArtCanvas 当前保留与上游 `@basketikun/canvas-agent` 的协议兼容，但本仓库不配置 npm 自动发布；需要验证 ArtCanvas 中的 Agent 源码时，请在本目录执行 `npm ci && npm run build` 后直接运行 `node dist/index.js`。
...
使用时可以直接在 Codex 里说“打开 ArtCanvas”，插件会启动本地 Agent，读取 Local URL 和 Connect token，然后在右侧打开 `https://canvas.uniart.fun/` 并自动新建、连接画布；只有明确要求使用本地项目时才会启动本地前端。

...
export function checkVersions() {
    ...
    if (!localCodexVersion) {
        ...
        logger.warn(`Bundled Codex ${CODEX_VERSION} does not match local Codex ${localCodexVersion}. Update the local CLI with: npm install -g @openai/codex@latest`);
    }
    void checkLatestVersions(localCodexVersion);
}

/** 查询 npm，提醒升级 Codex；ArtCanvas Agent 本身由仓库版本管理。 */
async function checkLatestVersions(localCodexVersion: string) {
    try {
        const latestCodex = await npmVersion("@openai/codex");
        if (isOlder(CODEX_VERSION, latestCodex)) logger.warn(`Update available: bundled Codex ${CODEX_VERSION} -> ${latestCodex}. Update ArtCanvas Agent from the repository when a newer integration is released.`);

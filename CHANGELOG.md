...
+ [修复] UniArt 视频模型按实际能力显示时长、比例和分辨率，并使用受支持的请求参数提交任务。
...
+ [修复] 视频参考素材改用与提交顺序一致的全局 `@n` 索引，避免图片、视频和音频分别编号导致引用错位。
...
+ [修复] 切换 UniArt Seedance 2.0 Official、VIP、Proxy 及 Fast 分层 SKU 后继续按 SKU 能力显示参考方式、素材入口和参数档位。
## v0.11.0 - 2026-07-28
